(function () {
    ...
    function fetchLogs(scriptId, requestId) {
        if (typeof require !== 'function' || !scriptId) {
            sendMessageToExtension('logs_error', 'RequireJS o scriptId faltante', null, { requestId });
            return;
        }
        require(['N/query'], function (query) {
            ...
            try {
                const rs = query.runSuiteQL({ query: sql, params: [scriptId] });
                const rows = rs.asMappedResults();
                sendMessageToExtension('logs_success', null, rows, { requestId });
            } catch (e) {
                sendMessageToExtension('logs_error',
    ...
    function handleCustomList(custListId) {
        ...
        require(['N/query'], function (query) {
            try {
                const sql = `SELECT name, id, fieldvaluetype, fieldvaluetyperecord, lower(scriptid) fieldid 
                            FROM customfield 
                            WHERE fieldvaluetype = 'List/Record' 
                            AND fieldvaluetyperecord = ?`;

                const resultSet = query.runSuiteQL({ query: sql, params: [custListId] });
                const results = resultSet.asMappedResults();
                ...
                sendMessageToExtension('error', innerError.message);
            }
        });
    }
